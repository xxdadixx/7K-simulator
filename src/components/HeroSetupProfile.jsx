...
// ย้าย Helper Functions ของสีและธาตุมาไว้ที่นี่เพราะใช้แค่ในนี้
        ...
        <div className="p-6 flex flex-col gap-6">
          ...
          <div key={activeHero.name} className="flex flex-col items-center justify-center -mt-2 animate-hero-swap">
            <div className={`relative w-[120px] aspect-[156/194] md:w-[140px] rounded-2xl overflow-hidden border-2 shadow-lg flex items-center justify-center transition-colors duration-300 ${getGradeBgClass(activeHero.grade)}`}>
              <img
                ...
                loading="lazy"
                decoding="async"
                className={`object-contain z-10 transition-all duration-500 drop-shadow-md ${activeHero.name === 'Unselected' ? 'w-12 h-12 opacity-20 grayscale' : 'w-full h-full hover:scale-110'}`}
                onError={(e) => {
            ...
            <h3 className={`mt-4 uppercase tracking-widest transition-colors ${getGradeColorClass(activeHero.grade)}`} style={{ fontFamily: '"Press Start 2P", monospace', fontSize: '1rem', textShadow: '0 0 8px currentColor, 0 0 15px currentColor' }}>
              {activeHero.name}
            </h3>
          </div>
          ...
          <div className="flex gap-4 w-full">
            <div className="flex-1 min-w-0">
              ...
              {/* 🌟 2. ลบคำว่า 30 (MAX) ออกเมื่อไม่มีฮีโร่ 🌟 */}
